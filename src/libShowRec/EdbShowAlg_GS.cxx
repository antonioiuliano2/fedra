		...
		for (Int_t pat_two_cnt=0; pat_two_cnt<npat; ++pat_two_cnt) {
			...
			for (Int_t pat_one_bt_cnt=0; pat_one_bt_cnt<pat_one_bt_cnt_max; ++pat_one_bt_cnt) {
				...
				for (Int_t pat_two_bt_cnt=0; pat_two_bt_cnt<pat_two_bt_cnt_max; ++pat_two_bt_cnt) {
					...
					// d) Check if smaller IP (of both) to vtx (BT) is ok:
					...
					IP_Pair_To_InBT_Seg = TMath::Min(IP_Pair_To_InBT_Seg,IP_Pair_To_InBT_Seg2);
					// Save the segment which has smaller IP, this will be the first BT in the RecoShower
					if ( IP_Pair_To_InBT_Seg>IP_Pair_To_InBT_Seg2 ) IP_Pair_To_InBT_SegSmaller=0; // take Segment first
						if ( IP_Pair_To_InBT_Seg<=IP_Pair_To_InBT_Seg2 ) IP_Pair_To_InBT_SegSmaller=1; // take Segment2 first
