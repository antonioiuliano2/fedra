...
EdbShowRec::EdbShowRec(EdbPVRec* gAli, TObjArray* InBTArray)
{
  ...
  eUse_LT=kFALSE;  eUse_AliBT=kFALSE;   eUse_AliLT=kFALSE;  eUse_PVREC=kFALSE;
  eUseNr=-1;
  SetPlateNumberValues();
  CheckPlateValues();
  /// STILL DEBUG STATUS....... IS THIS TRUE???-------------------------------
...
//______________________________________________________________________________
/// Strangely this constructor doenst work... If I use like this:
...
void EdbShowRec::Set0()
{
  ...
  for(int i=0; i<7; i++) eParaTypes[i]=0;
	...
	eParaNames[0]="FJ";eParaNames[1]="LT";
  eParaNames[2]="YC";eParaNames[3]="JC";
  eParaNames[4]="XX";eParaNames[5]="YY";
	eParaNames[6]="PP";
  eInBTTree=0;
...
void EdbShowRec::Init()
{
  ...
  eActualAlg=0;
  for (int k=0;k<10; k++) { eActualAlgParameterset[k]=0;}
...
//   delete RecoAlg;
  ...
  cout << "EdbShowRec::Reconstruct   NewRecoAlg->Execute()   finished" << endl;
  cout << eRecoShowerArray->GetEntries() << endl;
...
//   SetTreebranchName
  ...
  for (int i=0; i<eInBTArrayN; i++) {
    ...
    show->PrintNice();
    show->PrintSegments();
		//cout << show->GetNBT() <<"  " <<  show->GetNBTMC() << endl;
...
//   return;
  ...
  cout << "eRecoShowerArray->IsA() : " << eRecoShowerArray->IsA() << endl;
...
void EdbShowRec::Create_eInBTTree()
{
  ...
  if (eInBTTree) { delete eInBTTree; eInBTTree=0; cout << "deletion done"<<endl;}
  EdbSegP* seg=0;
...
void  EdbShowRec::Fill_eInBTArray(Int_t FillType)
{
  ...
  switch (FillType)
  { 
    case -1 : cout << "EdbShowRec::Fill_eInBTArray   FillType= " <<  FillType << " Nothing done by default....!" << endl;
    break;
    case 0 :  Fill_eInBTArray_ByLinkTracks_eFilename_LinkedTracks();
    break;
    case 1 :  Fill_eInBTArray_ByRecoLinkTracks_eAli();
    break;
    case 2 :  Fill_eInBTArray_ByBaseTracksOf_eAli();
    break;
    default : Fill_eInBTArray_ByRecoLinkTracks_eAli();
    break;
  }
...
void EdbShowRec::Fill_eInBTArray_ByLinkTracks_eFilename_LinkedTracks()
{
  ...
  if (gEDBDEBUGLEVEL>2) {
    ...
    EdbSegP* testseg=0;
    for(int i=0; i<eInBTArrayN; i++ ) {
      testseg=(EdbSegP*)eInBTArray->At(i); testseg->PrintNice();
    }
  }
...
void EdbShowRec::Fill_eInBTArray_ByBaseTracksOf_eAli()
{
  ...
  EdbSegP* testseg=0;
  for(int i=0; i<eInBTArrayN; i++ ) {
    if (gEDBDEBUGLEVEL>2) { testseg=(EdbSegP*)eInBTArray->At(i); testseg->PrintNice(); }
  }
...
void EdbShowRec::Reset()
{
  ...
  if (eAli)                             { eAli->Clear();  delete eAli;  eAli=0; }
  cout << "EdbShowRec::Reset()   Delete eInBTArray." << endl;
  if (eInBTArray)                       { eInBTArray->Clear();  delete eInBTArray;  eInBTArray=0; }
  cout << "EdbShowRec::Reset()   Delete eShowAlgArray." << endl;
  if (eShowAlgArray)                       { eShowAlgArray->Clear();  delete eShowAlgArray;  eShowAlgArray=0; }
  ...
  while (fgets(buf, sizeof(buf), fp)) {
    for (Int_t i = 0; i < (Int_t)strlen(buf); i++) 
      ...
      else if ( !strcmp(key,"SHOW_USE_LT")   )
      {
        ...
        ResetUse_InBTType(); 
        if (int_eUse==1) { 
          eUse_LT=kTRUE; eUse_PVREC=kFALSE; eUse_AliBT=kFALSE; eUse_AliLT=kFALSE;
          eUseNr=0;
        }
        else { 
          eUse_LT=kFALSE; 
      ...
      else if ( !strcmp(key,"SHOW_USE_ALIBT")   )
      {
        ...
        ResetUse_InBTType(); 
        if (int_eUse==1) { 
          eUse_LT=kFALSE; eUse_PVREC=kFALSE; eUse_AliBT=kTRUE; eUse_AliLT=kFALSE;
          eUseNr=2;
        }
        else { 
          eUse_AliBT=kFALSE; 
      ...
      else if ( !strcmp(key,"SHOW_USE_ALILT")   )
      {
        sscanf(buf+strlen(key),"%d",&int_eUse);
        ResetUse_InBTType(); 
        if (int_eUse==1) { 
          eUse_LT=kFALSE; eUse_PVREC=kFALSE; eUse_AliBT=kFALSE; eUse_AliLT=kTRUE;
          eUseNr=3;
        }
        else { 
          eUse_AliLT=kFALSE; 
      ...
      else if ( !strcmp(key,"SHOW_USE_PVREC")   )
      {
        ...
        ResetUse_InBTType(); 
        if (int_eUse==1) { 
          eUse_LT=kFALSE; eUse_PVREC=kTRUE; eUse_AliBT=kFALSE; eUse_AliLT=kFALSE;
          eUseNr=1;
        }
        else { 
          eUse_PVREC=kFALSE; 
      ...
      {
        float par[10]; for (int i=0; i<10; ++i) { par[i]=-9999.0; }
        int type;
        int n=sscanf(buf+strlen(key),"%d %f %f %f %f %f",&type, &par[0], &par[1], &par[2], &par[3], &par[4] );
        cout << "n= " << n << endl;
        if (n==5)  { AddAlg(type,par); }
        else { cout << "EdbShowRec::ReadShowRecPar   WRONG FORMAT of SHOW_ADDPARASET. 4 Parameters at least!" << endl; }
      }
      ...
      else if ( !strcmp(key,"SHOW_SETPARASET")  )
      {
        ...
        int n=sscanf(buf+strlen(key),"%d %f %f %f %f %f",&type, &par[0], &par[1], &par[2], &par[3], &par[4] );
        cout << "n= " << n << endl;
        if (n==5)  { ResetShowAlgArray(); AddAlg(type,par); }
        else { cout << "EdbShowRec::ReadShowRecPar   WRONG FORMAT of SHOW_ADDPARASET. 4 Parameters at least!" << endl; }
      }
...
void EdbShowRec::ResetUse_InBTType()
{
  ...
  eUse_LT=kFALSE; eUse_PVREC=kFALSE; eUse_AliBT=kFALSE; eUse_AliLT=kFALSE; 
  eUseNr=-1;
...
void EdbShowRec::Add_INBTCut(int layer, TCut &cut)
{
  ...
  if(layer<0) return;
  if(layer>2) return;
  if(!eInBTCuts[layer]) { cout << " EdbShowRec::Add_INBTCut   No eInBTCuts[layer] yet"<<endl; eInBTCuts[layer] = new TCut(cut); }
...
void EdbShowRec::TxtToRecoShowerArray(TString TxtFileName, Int_t TxtFileType)
{
  ...
  switch(TxtFileType)
    {
    case 0: cout << "READ 0) Simple Style." << endl;
	    TxtToRecoShowerArray_SimpleList(TxtFileName);
	      break;

    case 1: cout << "READ 1) EdaStyle." << endl;
	    TxtToRecoShowerArray_EDAList(TxtFileName);
	      break;
    
    case 2: cout << "READ 2) __Official__ (18.05.2010) Feedback Style." << endl; 
	    TxtToRecoShowerArray_FeedBack(TxtFileName);
	      break;
    
    case 3: cout << "READ 3) NagoyaTxtStyle." << endl;
	    TxtToRecoShowerArray_SimpleListNagoya(TxtFileName);
	      break;	      
    
    default: cout << "DO NOTHIN!" << endl;
    }
  Log(2,"EdbShowRec::TxtToRecoShowerArray","EdbShowRec::TxtToRecoShowerArray...done.");
}
...
{
	Log(2,"EdbShowRec::TxtToRecoShowerArray_FeedBack","EdbShowRec::TxtToRecoShowerArray_FeedBack");
	...
	if(fp==NULL) {
		Log(2,"EdbShowRec::TxtToRecoShowerArray_FeedBack","EdbShowRec::File open error.");
		return;
	}
...
void EdbShowRec::TxtToRecoShowerArray_SimpleList(TString TxtFileName)
{
	...
	// Copied the skeleton from Ariga-San code from EdbEDAIO class,
	// and took out some unnnecessary stuff...
	// Read simple lsit file format (originally sent by Napoli People.).
...
void EdbShowRec::TxtToRecoShowerArray_EDAList(TString TxtFileName)
{
	...
	while(fgets(buf,sizeof(buf),fp)){
		...
		if (ntokens==0) continue;
			...
			float x,y,z,ax,ay;int id,pl,w,dummy;
...
// 			$8 : PH (scan data)
			...
			float x,y,z,ax,ay; int id,pid,w;
...
void EdbShowRec::RecoShowerArray_To_Treebranch()
{
  ...
  for (int i_shower=0; i_shower<showarray->GetEntries(); i_shower++) { 
  // START OVER LOOP of   ONE SHOWER:
    show=(EdbShowerP*)showarray->At(i_shower);
    
    if (i_shower%100==0) cout << "	EdbShowRec::RecoShowerArray_To_Treebranch   Doing:   "  << i_shower << "     with  eNBT=  " << show->N() << endl;
// 		cout << "	EdbShowRec::RecoShowerArray_To_Treebranch   Address of shower: " << show << endl;
  ...
  // Initialize arrays...
    ...
    test_shower_deltax=0;test_shower_deltay=0;
    max_diff_pid=0;
    extrapol_x=0;extrapol_y=0;extrapo_diffz=0;
    shower_sizeb15=0;shower_sizeb20=0;shower_sizeb30=0;
    shower_sizeb=0;shower_energy_shot_particle=0.0;shower_numberofilms=0;
		shower_numberofholes=0;shower_numberofholesconseq=0;
    for (int ii=0;ii<5000;ii++)  {
      shower_xb[ii]=0;shower_yb[ii]=0;shower_zb[ii]=0;
      shower_txb[ii]=0;shower_tyb[ii]=0;
      shower_nfilmb[ii]=0;shower_tagprimary[ii]=0;
      shower_ntrace1simub[ii]=0;shower_ntrace2simub[ii]=0;
      shower_ntrace3simub[ii]=0;shower_ntrace3simub[ii]=0;
      shower_deltaxb[ii]=0;shower_deltayb[ii]=0;
      shower_chi2btkb[ii]=0;
      shower_idb[ii]=0; shower_plateb[ii]=0;
    }
    for (int i=1;i<59;++i) {shower_deltasigmathetab[i]=0;}
...
//     cout << "Inseg->PrintNice();"<< endl;
//     Inseg->PrintNice();
    if (gEDBDEBUGLEVEL>3) cout << "---   ---   ---------------------"<<endl;
    
  //-------------------------------------
    for (int ii=0;ii<shower_sizeb;ii++)  {
      if (ii>=5000) { cout << "WARNING: shower_sizeb ( " << shower_sizeb<< ") greater than SHOWERARRAY.   Set sizeb to 4999 and  Stop filling!."<<endl; shower_sizeb=4999; break;}
    ...
    //-------------------------------------
      shower_xb[ii]=seg->X();shower_yb[ii]=seg->Y();
      shower_txb[ii]=seg->TX();shower_tyb[ii]=seg->TY();
      shower_zb[ii]=seg->Z();shower_chi2btkb[ii]=seg->Chi2();
      shower_deltathetab[ii]=0.5;shower_deltarb[ii]=200;
      shower_tagprimary[ii]=0; if (ii==0) shower_tagprimary[ii]=1;
      shower_isizeb=1; // always 1, not needed anymore
      if (seg->MCEvt()>0) shower_number_eventb=seg->MCEvt();
      shower_ntrace1simub[ii]=0; if (seg->MCEvt()>0) shower_ntrace1simub[ii]=seg->MCEvt();
      shower_ntrace2simub[ii]=seg->W();
      shower_ntrace3simub[ii]=seg->P();
      shower_ntrace4simub[ii]=seg->Flag();
      shower_idb[ii]=seg->ID(); shower_plateb[ii]=seg->PID();
    //-------------------------------------
...
// 	if (test_shower_deltar<150 && test_shower_deltathetab<0.15 ) {
    ...
    //-------------------------------------
  		...
  		//-------------------------------------
    for (int i=1;i<59;++i) {shower_deltasigmathetab[i]=TMath::Sqrt(shower_deltasigmathetab[i]);}
  ...
  Log(2,"EdbShowRec::RecoShowerArray_To_Treebranch","EdbShowRec::RecoShowerArray_To_Treebranch.Loop finished.");
  ...
  delete eShowerTree; eShowerTree=0;
  delete eFile_Out_treebranch; eFile_Out_treebranch=0;
  
  Log(2,"EdbShowRec::RecoShowerArray_To_Treebranch","EdbShowRec::RecoShowerArray_To_Treebranch...done.");
  return;
}
...
void EdbShowRec::AddAlg(Int_t AlgType, Float_t* par)
{
 ...
 // switch command gives compiler errors.... change to simple if statements:
  ...
  if (AlgType==0) {cout << "= = = = = CT not yet implemented = = = = = "<<endl; return; }; // CT
  if (AlgType==1) {cout << "= = = = = CL not yet implemented = = = = = "<<endl; return; }; // CL
  if (AlgType==2) {EdbShowAlg_CA* StandardAlg = new EdbShowAlg_CA(); eShowAlgArray->Add(StandardAlg); StandardAlg->SetParameters(par);StandardAlg->Print();};
  if (AlgType==3) {cout << "= = = = = NN not yet implemented = = = = = "<<endl; return; }; // NN
	if (AlgType==4) {EdbShowAlg_OI* StandardAlg = new EdbShowAlg_OI(); eShowAlgArray->Add(StandardAlg); StandardAlg->SetParameters(par);StandardAlg->Print();};
  if (AlgType==5) {EdbShowAlg_SA* StandardAlg = new EdbShowAlg_SA(); eShowAlgArray->Add(StandardAlg); StandardAlg->SetParameters(par);StandardAlg->Print();};
	if (AlgType==6) {cout << "= = = = = TC not yet implemented = = = = = "<<endl; return; }; // TC
	if (AlgType==7) {EdbShowAlg_RC* StandardAlg = new EdbShowAlg_RC(); eShowAlgArray->Add(StandardAlg); StandardAlg->SetParameters(par);StandardAlg->Print();};
...
void EdbShowRec::BuildParametrizations()
{
  ...
  for(int i=0; i<eRecoShowerArrayN; i++ ) {
    ...
    for( int j=0; j<maxParametrizationType; j++ ) { if (eParaTypes[j]==1) show->BuildParametrisation(j); }
  }
}
...
{
  if (StreamTypeStep==1) { eFirstPlate_eAliPID = eAliNpat-FP; }
  if (StreamTypeStep==-1) { eFirstPlate_eAliPID = FP-1; }
...
{
  if (StreamTypeStep==1) { eMiddlePlate_eAliPID = eAliNpat-Plate; }
  if (StreamTypeStep==-1) { eMiddlePlate_eAliPID = Plate-1; }
...
{
  if (StreamTypeStep==1) { eLastPlate_eAliPID = eAliNpat-Plate; }
  if (StreamTypeStep==-1) { eLastPlate_eAliPID = Plate-1; }
...
void EdbShowRec::WriteParametrisation_FJ() {
  ...
  Float_t BT_deltaR_mean,BT_deltaR_rms,BT_deltaT_mean,BT_deltaT_rms; Int_t nseg;
...
//     cout << "GGGGGGGGGGGGGG       ShowerAxisAngle= " <<  ShowerAxisAngle << endl;
    nseg=para_FJ.nseg; BT_deltaR_mean=para_FJ.BT_deltaR_mean;  BT_deltaR_rms=para_FJ.BT_deltaR_rms;
    BT_deltaT_mean=para_FJ.BT_deltaT_mean;  BT_deltaT_rms=para_FJ.BT_deltaT_rms;
  ...
  if (gEDBDEBUGLEVEL>3) { treevar->Print(); }
	//------------------------------------------------------------------
...
void EdbShowRec::WriteParametrisation_YC() {
  ...
  Float_t ShowerAxisAngle,C1,a1,alpha,nmax; Int_t nseg;
  TTree* treevar=0;
  if (!treevar) treevar = new TTree(targettreename,targettreename);
	treevar->Branch("ShowerAxisAngle",&ShowerAxisAngle,"ShowerAxisAngle/F");
  ...
  for(int i=0; i<eRecoShowerArrayN; i++ ) {
    ...
    //Fill tree with values from parametrisation:
    ShowerAxisAngle=para_YC.ShowerAxisAngle;C1=para_YC.C1;a1=para_YC.a1;alpha=para_YC.alpha;nmax=para_YC.nmax;nseg=para_YC.nseg;
...
void EdbShowRec::WriteParametrisation_LT() {
  ...
  Float_t ShowerAxisAngle;
  Float_t BT_deltaR_mean,BT_deltaR_rms,BT_deltaT_mean,BT_deltaT_rms; Int_t nseg;
  Int_t longprofile[57];Int_t transprofile[8];
  ...
  for(int i=0; i<eRecoShowerArrayN; i++ ) {
    ...
    nseg=para_LT.nseg; BT_deltaR_mean=para_LT.BT_deltaR_mean;  BT_deltaR_rms=para_LT.BT_deltaR_rms;
    BT_deltaT_mean=para_LT.BT_deltaT_mean;  BT_deltaT_rms=para_LT.BT_deltaT_rms;
...
void EdbShowRec::WriteParametrisation_XX() {
  ...
  Int_t nseg; 	Float_t ShowerAxisAngle;
...
// cout << "   IRGENDAWS IST HIER FALSCH MIT BEM BRACH FILLING !!!  SEEMS TO BE SOLVED SINCE ARRAY FILLING WAS WRONG! "<< endl;
  ...
  for(int i=0; i<eRecoShowerArrayN; i++ ) {
    ...
    // Do parametrisation if necessary:
    if (!ToDoPara) { show->BuildParametrisation(5); }
...
void EdbShowRec::WriteParametrisation_SE() {
  ...
  Int_t nseg,npl; 	Float_t ShowerAxisAngle,efficiencyAtShowerAxisAngle;
...
// 	cout << "PGunTree->GetEntries();    " <<  PGunTree->GetEntries() <<  endl;   
  ...
  for (Int_t i=0; i<eRecoShowerArrayN; ++i) {
   ...
   fMCInfo_PGun.energy=energy;   fMCInfo_PGun.tantheta=tantheta;
   fMCInfo_PGun.dirx=dirx;   fMCInfo_PGun.diry=diry;   fMCInfo_PGun.dirz=dirz;
	 fMCInfo_PGun.vtxposx=vtxposx;   fMCInfo_PGun.vtxposy=vtxposy;   fMCInfo_PGun.vtxposz=vtxposz;
   fMCInfo_PGun.TX=TX;   fMCInfo_PGun.TY=TY;
   fMCInfo_PGun.X=X;   fMCInfo_PGun.Y=Y;   fMCInfo_PGun.Z=Z;
...
void EdbShowRec::WriteParametrisation_MCInfo_PGun()
{
  ...
  for(int i=0; i<eRecoShowerArrayN; i++ ) {
    ...
    //Fill tree with values from parametrisation:
    MCEvt=fMCInfo_PGun.MCEvt;    PDGId=fMCInfo_PGun.PDGId;
    energy=fMCInfo_PGun.energy;    tantheta=fMCInfo_PGun.tantheta;
    dirx=fMCInfo_PGun.dirx;    diry=fMCInfo_PGun.diry;    dirz=fMCInfo_PGun.dirz;
		vtxposx=fMCInfo_PGun.vtxposx;vtxposy=fMCInfo_PGun.vtxposy;vtxposz=fMCInfo_PGun.vtxposz;
    X=fMCInfo_PGun.X;    Y=fMCInfo_PGun.Y;    Z=fMCInfo_PGun.Z;
    TX=fMCInfo_PGun.TX;    TY=fMCInfo_PGun.TY;
