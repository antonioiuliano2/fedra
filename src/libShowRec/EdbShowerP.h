...
class EdbShowerP : public EdbSegP {
	...
	public:
		...
		struct MCInfo_PGun {
			...
			Int_t MCEvt;      Int_t PDGId;
			Float_t energy;      Float_t tantheta;
			Float_t dirx; Float_t diry; Float_t dirz;
			Float_t vtxposx; Float_t vtxposy; Float_t vtxposz;
			Float_t TX;      Float_t TY;
			Float_t Y;      Float_t X;      Float_t Z;
		...
		void        Set0();
		EdbTrackP   *TrackAS() const {return eTrackAS;}
		...
		inline void    SetCounters()           { N(); SetNpl(); SetN0(); SetN00(); }
		inline void    SetN0( int n0 )         { eN0 = n0; }
		inline void    SetN0()                 { eN0 = eNpl-N(); }
		inline Int_t   N0()      const         { return eN0; }
		inline void    SetNpl( int npl )       { eNpl=npl; }
		inline void    SetNpl()                { if(eS) eNpl = 1+TMath::Abs(GetSegment(0)->PID() - GetSegment(N()-1)->PID()); }
		inline Int_t   Npl()     const         { return eNpl; }
		inline Int_t   N()       const         { return (eS) ?  eS->GetEntries()  : 0; }
		inline void    SetN00( int n00 )         { eN00 = n00; }
		inline void    SetN00()                 { eN00 = eNpl-N(); }
		inline Int_t   N00()      const         { return eN00; }
		
		inline TObjArray* GetSegmentArray()    { return eS; }

		inline EdbSegP *GetSegmentFirst()  const {return (eS) ? (EdbSegP*)(eS->First()) : 0;}
		inline EdbSegP *GetSegmentLast()   const {return (eS) ? (EdbSegP*)(eS->Last()) : 0;}
		inline EdbSegP *GetSegment(int i)  const {return (eS) ? (EdbSegP*)(eS->At(i)) : 0; }

		inline Float_t GetSegmentFirstZ()  const {return (eS) ? ((EdbSegP*)eS->First())->Z() : 9999999;}
		inline Float_t GetSegmentLastZ()   const {return (eS) ? ((EdbSegP*)eS->Last())->Z() : 9999999;}
		inline Float_t GetFirstZ()  const {return eFirstZ;}
		inline Float_t GetLastZ()   const {return eLastZ;}
		...
		// Gravity Center, Axis:
		inline EdbSegP* GetSegmentGravity()  const {return eShowerAxisCenterGravityBT;}
		inline Float_t  GetSegmentAngle()    const {return eShowerAxisAngle;}
		...
		inline void    SetSegmentArray(TObjArray* segArray)     { eS=segArray; Update(); }
		...
		inline Int_t   GetNBT()       const         { return eNBT; }
		inline Int_t   GetNBTMC()     const         { return eNBTMC; }
		inline Int_t   GetNBTMCFlag(Int_t PdgArray)     const         { return eNBTMCFlag[PdgArray]; }
		inline Int_t   GetNBTPlate(Int_t PIDdiff)      const         { return eNBTPlate[PIDdiff]; }
		inline Int_t   GetNBTMCPlate(Int_t PIDdiff)    const         { return eNBTMCPlate[PIDdiff]; }
		inline Int_t   GetNBTMCFlagPlate(Int_t PIDdiff,Int_t PdgArray)     const { return eNBTMCFlagPlate[PIDdiff][PdgArray]; }

		inline Float_t GetPurity()        const         { return ePurity; }
		inline Float_t GetSphericity()    const         { return eSphericity; }
		inline TH1F*   GetProfileLongitudinal()   const { return eProfileLongitudinal; }
		inline TH1F*   GetProfileTransversal()   const { return eProfileTransversal; }
		TH1F*          GetProfileLongitudinalPlate();

		inline Bool_t   GetParametrisationIsDone(Int_t nr) const  { return eParametrisationIsDone[nr]; }
		Float_t         GetParaVariable(Int_t ParaVariable, Int_t ParaNr);

		inline Para_FJ   GetPara_FJ()       const         { return ePara_FJ; }
		inline Para_LT   GetPara_LT()       const         { return ePara_LT; }
		inline Para_YC   GetPara_YC()       const         { return ePara_YC; }
		inline Para_XX   GetPara_XX()       const         { return ePara_XX; }
		inline Para_YY   GetPara_YY()       const         { return ePara_YY; }
		inline Para_PP   GetPara_PP()       const         { return ePara_PP; }
		inline Para_AS   GetPara_AS()       const         { return ePara_AS; }
		inline Para_SE   GetPara_SE()       const         { return ePara_SE; }
		inline Para_ExtraInfo   GetPara_ExtraInfo()       const         { return ePara_ExtraInfo; }
		...
		inline Bool_t        GetMCInfoIsDone(Int_t nr) const  { return eMCInfoIsDone[nr]; }
		inline Bool_t        GetExtraInfoIsDone(Int_t nr) const  { return eExtraInfoIsDone[nr]; }
		inline MCInfo_PGun   GetMCInfo_PGun()       const         { return eMCInfo_PGun; }
		inline MCInfo_Event  GetMCInfo_Event()       const         { return eMCInfo_Event; }

		inline void          SetMCInfoIsDone(Int_t nr, Bool_t IsDone) {  eMCInfoIsDone[nr]=IsDone; return; }
		inline void          SetExtraInfoIsDone(Int_t nr, Bool_t IsDone) {  eExtraInfoIsDone[nr]=IsDone; return; }
		inline void          SetMCInfo_PGun(MCInfo_PGun fMCInfo_PGun) {eMCInfo_PGun=fMCInfo_PGun;}
		inline void          SetMCInfo_Event(MCInfo_Event fMCInfo_Event) {eMCInfo_Event=fMCInfo_Event;}
