...
//	number_eventb = a->GetID();
	...
	if(isizeb==1)
	{
//	    if (fabs(a->GetZ0()-ZZ0[index])<8000)printf("%f %f \n",fabs(a->GetZ0()-ZZ0[index]),ZZ0[index]); 
	    if (fabs(a->GetZ0()-ZZ0[index])>19200) isizeb=15;
	    if (fabs(a->GetZ0()-ZZ0[index])>25700) isizeb=20;
	    if (fabs(a->GetZ0()-ZZ0[index])>38700) isizeb=30;
	    if (fabs(a->GetZ0()-ZZ0[index])>64700) isizeb=50;
	}
	for (int j=0;j<add[i]; j++) 
	{    
	    xb[j] = a->GetXb(j);
