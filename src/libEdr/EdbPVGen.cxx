			...
			EdbTrackP &tr, int eloss_flag, float PGap)
{
  ...
  for (int k=0; k<ePVolume->Npatterns(); k++ ) {
    ...
    seg->Set(tr.ID(), xs, ys, txs, tys,25,0);
    seg->SetP(p);
    seg->SetZ(z);
    seg->SetDZ(300.);
    seg->SetPID(k);
...
//______________________________________________________________________________
void EdbPVGen::GenerateUncorrelatedSegments(int nb, float lim[4], float TetaMax, int flag )
{
					 ...
					 int prim_charge )
{
  ...
  for(int iv=0; iv<nv; iv++) {
	...
	tx = pi->Px()/pi->Pz();
	ty = pi->Py()/pi->Pz();
	tr->Set(numt++, x, y, (float)tx, (float)ty,1,0);
	tr->SetZ(z);
	tr->SetP(pi->P());
	tr->SetM(pi->M());
	tr->SetFlag(iv+1);
