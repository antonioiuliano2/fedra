...
void print_View(int side, int i)
{
   ...
   //if (i<2 || i>848)
      cout << side <<" "<< id <<" "<< topz <<" "<< bottomz <<" "<< px <<" "<< py << endl;
} ;
//___________________________________________________________________________//
...
void printrackIndexEntry(int s, int i )
{
   ...
   cout << s <<" "<< i <<" "<< TrackIndexEntry::Fragment <<" "
...
void Add_MIPBaseTrack(int i, float project_dz= -105.)
{
      ...
      s->SetChi2( Sigma ) ;
      //s->SetVolume( AreaSum ) ;  // non c'e' nei couples di fedra
      s->SetZ( z_ ) ;
                  ...
                  float project_dz= -105. ) 
{
   ...
   if (headerformat == (UShort_t) File::Format::Detailed)
	{
