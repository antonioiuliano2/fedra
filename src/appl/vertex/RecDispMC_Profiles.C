...
TCanvas *cv = 0, *cv1 = 0, *cv2 = 0, *cv4 = 0;
...
void gv(int n=1)
{
  ...
  scanCond->SetSigma0( .5, .5,.0015,.0015 );  // sigma0 "x,y,tx,ty" at 0 angle
...
void gen_segs_v(BRICK &b, EdbVertex &vedb)
{
  ...
  for(int ip=0; ip<nt; ip++) {
    ...
    gener->TrackMC( zlim, b.lim, seg_s, *tr, eloss_flag, 0.);
    gRandom->SetSeed(seed);
    gener->TrackMC( zlim, b.lim, seg_zero, *ti[ip], eloss_flag, 0.);
  }
}
...
void rec_v(EdbVertex &vedb)
{
  ...
  for(int ip=0; ip<nt; ip++) {
	...
	for (int is = 0; is < brick.plates; is++)
	{
	    ...
	    dx = sf->X() - s->X();
	    dtx = sf->TX() - s->TX();
	    hpp[2]->Fill((float)is,dx/TMath::Sqrt(seg_s[0]*seg_s[0] - sx*sx));
	    hpp[3]->Fill((float)is,dtx/TMath::Sqrt(seg_s[2]*seg_s[2] - stx*stx));
...
void BookHistsV()
{
  if (!hp[0]) hp[0] = new TH1F("Hist_Vt_DtrackX","Track Delta X, microns",100,-5.,+5.);
  if (!hp[1]) hp[1] = new TH1F("Hist_Vt_DtrackTX","Track Delta TX, rad",100,-0.010,+0.010);
  if (!hp[2]) hp[2] = new TH1F("Hist_Vt_DsegX","Segment Delta X, microns",100,-5.,+5.);
  if (!hp[3]) hp[3] = new TH1F("Hist_Vt_DsegTX","Segment Delta TX, rad",100,-0.050,+0.050);
  if (!hp[4]) hp[4] = new TH1F("Hist_Vt_StrackX","Track Sigma X, microns",100,0.,+5.);
  if (!hp[5]) hp[5] = new TH1F("Hist_Vt_StrackTX","Track Sigma TX, rad",100,0.,+0.010);
  if (!hp[6]) hp[6] = new TH1F("Hist_Vt_Prob","Chi Square Probability",26,0.,1.04);
