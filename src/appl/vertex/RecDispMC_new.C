...
bool  mc_make_tracks  = false;	     // build tracks from MC information
bool  use_mc_momentum = false;        // use or not MC track momentum
...
float back2_plim[2] = {0.5, 5.};    // momentum range for cosmics

float fiducial_border = 50000.;      // Working brick area is:
float fiducial_border_z = 17000.;    // xmin = brick.xmin + fiducial_border
...
void gv(int n=1, int nve=100, float back1=0., float back2=0.)
{
  ...
  gEVR->eDZmax      = 4500.;     // default is 3000. microns
  gEVR->eProbMin    = ProbMinV;  // default 0.01, i.e 1%
  gEVR->eImpMax     = 35.;       // default is 25. microns
...
void rec_all()
{
  ...
  for (int itr=0; itr<ntr; itr++) {
    ...
    if (ntrg)
    {
	itrg = tr->GetSegmentsMCTrack(nsegmatch);
	if (itrg >= 0 && itrg < ntrg)
	{
	    ...
	    tr->GetSegmentFirst()->SetErrorP(p*p*0.2*0.2);
	    tr->GetSegmentLast()->SetErrorP(p*p*0.2*0.2);
	    if (use_mc_momentum)
	    {
  ...
  // merge tracks
  for (int i=0; i<10; i++)
    if (ali->PropagateTracks(brick.plates-1,2,ProbMinP, 3) < 1) break;
...
//	float pms = tr->P_MS(brick.X0, trg->M(), true);
  ...
  if (nprong > 2 || read_from_file) nadd = gEVR->ProbVertexN();
		...
		if (v->valid())
		{
		    if ((np = v->ntracks()) >= 2)
		    {
				...
				else
				{
			        ...
			        if (np == 5 ) printf("Found 5-prongs vert %d tr (%d) ivg %d\n", j+1, tr->ID(), ivg);
				if (ivg != ivg0) break;
//			        if (np == 5 && ivg > 0) printf("Found 5-prongs vert %d tr good!\n", j+1);
			    }
			    ...
			    if (np == 5) printf("Found 5-prongs gen vert!\n");
			    ...
			    if (!read_from_file && ntold == 0) negood_tot++;
			    else if (edbvg->Flag() == 1 && ntold == 0) negood_tot++; 
			    vxo = edbv->VX();	
			    vyo = edbv->VY();	
			    vzo = edbv->VZ();
			    vxg = edbvg->X();
...
//		vc->Print();
    ...
    {
	int nn = gEVR->VertexNeighbor(1000., 2, 10000.);
	printf("------------------------------------------------------------\n");
	printf("%d neighboring tracks and segments found\n", nn);
    }
  }
}
...
void dsst(int numt = -1)
{
  ...
  const char *title = "display-stopped-tracks";
  if(!(ds = EdbDisplay::EdbDisplayExist(title))) 
  ...
  for(int i=it0; i<it1; i++) 
  {
    tra = (EdbTrackP*)(ali->eTracks->At(i));
    if (!tra) continue;
    if (tra->Flag() != -10 && tra->N() >= 20 && tra->N() <= 50) // skip "broken" tracks
    {
	if (tra->TrackZmin()->PID() > 10) continue;
	arrtr->Add(tra);
	nn = gEVR->SegmentNeighbor(tra->TrackZmax(), 8000., 5, arrs, 0);
...
void dsv( int numv = -1, int ntrMin=0 )
{
  ...
  for(int iv=iv0; iv<iv1; iv++)  {   
	...
	if (!seg) continue;
	arrs->Add(seg);
        printf("       Neighbor Segment ID %d, Z = %f\n",
               seg->ID(), seg->Z());
      }
	...
	if (!track) continue;
	arrtr->Add(track);
        printf("       Neighbor Track ID %d, Z = %f, Nseg = %d, Zpos = %d\n",
...
void dsvg2( int numv = -1 )
{
  ...
  const char *title = "display-generated-vertexes-2";
  if(!(ds2 = EdbDisplay::EdbDisplayExist(title))) 
    ds2=new EdbDisplay(title,-60000.,60000.,-60000., 60000., 0.,80000.);
