{
    ...
    bool stop=kFALSE;
    while (!stop) {
        cut_gs_cut_dip=gRandom->Uniform(50,200);
        cut_gs_cut_dmin=gRandom->Uniform(0,50);
        cut_gs_cut_dr=gRandom->Uniform(50,300);
        cut_gs_cut_dz=gRandom->Uniform(0,4000)+4000;
        cut_gs_cut_dtheta=gRandom->Uniform(0,0.25)+0.02;
        cut_gs_cut_piddiff=Int_t(gRandom->Uniform(0,4));
